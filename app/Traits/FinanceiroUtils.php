            ...
            $cotas = FinanceiroLancamento::
                select(
                    DB::raw("(SELECT SUM(valor) FROM financeiro_lancamentos fl
                        ...
                        WHERE fpc.numeracao in ('1.01.01', '1.01.02', '1.02.01') AND fl.instituicao_id = $instituicao_id $sqlDataMovimento AND conciliado = 1 AND fl.deleted_at is null) AS dizimos_ofertas"),
                    DB::raw("(SELECT SUM(valor) FROM financeiro_lancamentos fl
                        JOIN financeiro_plano_contas fpc ON fpc.id = fl.plano_conta_id
