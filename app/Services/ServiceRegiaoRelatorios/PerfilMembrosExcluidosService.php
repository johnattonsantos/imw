...
class PerfilMembrosExcluidosService
{
    use Identifiable;
    ...
    public function execute(string $dataInicial, string $dataFinal): array
    {
        ...
        $membros = DB::table('membresia_rolpermanente as mr')
            ->join('membresia_membros as mm', 'mm.id', '=', 'mr.membro_id')
            ...
            ->leftJoin('membresia_situacoes as modo', function ($join) {
                $join->on('modo.id', '=', 'mr.modo_exclusao_id')
            ...
            ->whereBetween('mr.dt_exclusao', [$dataInicial, $dataFinal])
            ->select([
                ...
                'modo.nome as modo_exclusao',
                'mr.dt_exclusao as data_exclusao',
            ])
