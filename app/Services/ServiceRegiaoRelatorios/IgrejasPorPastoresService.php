...
class IgrejasPorPastoresService
{
    public function execute(array $params = []): array
    {
        ...
        $pastorId = $params['pastor'] ?? 'all';

        return [
            'regiao' => $regiao,
            'distritos' => Identifiable::fetchDistritosByRegiao($regiao->id),
            'igrejas' => $this->fetchIgrejas($regiao->id),
            'pastores' => $this->fetchPastores($regiao->id),
            'lancamentos' => $this->fetchLancamentos($regiao->id, $distritoId, $igrejaId, $pastorId),
        ];
    }

    private function fetchIgrejas(int $regiaoId)
    {
        $query = DB::table('instituicoes_instituicoes as igreja')
    ...
    private function fetchPastores(int $regiaoId)
    {
    ...
    private function fetchLancamentos(int $regiaoId, string $distritoId, string $igrejaId, string $pastorId)
    {
        $query = DB::table('pessoas_nomeacoes as pn')
            ...
            ->when($pastorId !== 'all' && is_numeric($pastorId), function ($q) use ($pastorId) {
                $q->where('pp.id', (int) $pastorId);
            })
            ->orderBy('distrito.nome')
            ->orderBy('igreja.nome')
