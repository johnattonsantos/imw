...
@section('content')
    ...
    <div class="container-fluid d-flex justify-content-between">
        <div>
            <a href="{{ route('ebd.liderancas.create') }}" class="btn btn-primary position-relative mt-3 mb-3 ml-2">
                ...
                <span class="ml-2">NOVA LIDERANÇA</span>
            </a>
        </div>
    </div>
                        ...
                        <tbody>
                            @forelse ($liderancas as $item)
                                @php
                                    ...
                                    $celular = $item->membro->contato->telefone_preferencial
                                        ?? $item->membro->contato->telefone_whatsapp
                                        ?? $item->membro->contato->telefone_alternativo
                                        ?? null;
                                    $celularFormatado = $celular ? formatStr($celular, '(##) #####-####') : '-';
                                @endphp
                                <tr>
                                    ...
                                    <td class="d-flex" style="gap: .5rem;">
                                        <a href="{{ route('ebd.liderancas.edit', $item->id) }}" class="btn btn-sm btn-dark mr-2 btn-rounded bs-tooltip" title="Editar" aria-label="Editar">
