        ...
        <form class="form-vertical" action="{{ route('clerigos.perfil.dependentes.update', $dependente->id) }}" method="POST">
          ...
          <div class="row">
            ...
            <div class="form-group mb-4 col-12 col-sm-6 col-md-3">
              ...
              <input type="date" class="form-control @error('data_nascimento') is-invalid @enderror" id="data_nascimento" name="data_nascimento" value="{{ old('data_nascimento', $dependente->data_nascimento->format('Y-m-d')) }}" placeholder="{{ __('ex: 31/12/2000') }}">              @error('data_nascimento')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
          </div>
          ...
          <div class="row">
            <div class="form-group mb-4 col-12 col-sm-6 col-md-3">
              ...
              <select id="parentesco" name="parentesco" class="form-control @error('parentesco') is-invalid @enderror">
                <option value="" {{ old('parentesco', $dependente->parentesco) == '' ? 'selected' : '' }}>{{ __('Selecione') }}</option>
                <option value="Cônjuge" {{ old('parentesco', $dependente->parentesco) == 'Cônjuge' ? 'selected' : '' }}>{{ __('Cônjuge') }}</option>
                <option value="Filho(a)" {{ old('parentesco', $dependente->parentesco) == 'Filho(a)' ? 'selected' : '' }}>{{ __('Filho(a)') }}</option>
              </select>
              @error('parentesco')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            ...
            <div class="form-group mb-4 col-12 col-sm-6 col-md-3">
              ...
              <select id="sexo" name="sexo" class="form-control @error('sexo') is-invalid @enderror">
                <option value="" {{ old('sexo', $dependente->sexo) == '' ? 'selected' : '' }}>{{ __('Selecione') }}</option>
                <option value="M" {{ old('sexo', $dependente->sexo) == 'M' ? 'selected' : '' }}>{{ __('Masculino') }}</option>
                <option value="F" {{ old('sexo', $dependente->sexo) == 'F' ? 'selected' : '' }}>{{ __('Feminino') }}</option>
              </select>
              @error('sexo')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            ...
            <div class="form-group mb-4 col-sm-12 col-md-6">
              ...
              <select id="declarar_em_irpf" name="declarar_em_irpf" class="form-control @error('declarar_em_irpf') is-invalid @enderror">
                <option value="" {{ old('declarar_em_irpf', $dependente->declarar_em_irpf) == '' ? 'selected' : '' }}>{{ __('Selecione') }}</option>
                <option value="1" {{ old('declarar_em_irpf', $dependente->declarar_em_irpf) == '1' ? 'selected' : '' }}>{{ __('Sim') }}</option>
                <option value="0" {{ old('declarar_em_irpf', $dependente->declarar_em_irpf) == '0' ? 'selected' : '' }}>{{ __('Não') }}</option>
              </select>
