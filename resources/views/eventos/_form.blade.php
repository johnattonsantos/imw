...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="form-group">
            <label for="evento_proposito_id">{{ __('* Propósito') }}</label>
    ...
    <div class="col-md-4">
        <div class="form-group">
            <label for="titulo">{{ __('* Nome do Evento') }}</label>
...
<div class="row">
    <div class="col-md-3">
        <div class="form-group">
            <label for="data_inicio">{{ __('* Data Inicial') }}</label>
            <input type="text" name="data_inicio" id="data_inicio" class="form-control data-ptbr @error('data_inicio') is-invalid @enderror" value="{{ $dataInicio }}" placeholder="{{ __('dd/mm/aaaa') }}" autocomplete="off" required>
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            <label for="hora_inicio">{{ __('Hora Inicial') }}</label>
    ...
    <div class="col-md-3">
        <div class="form-group">
            <label for="data_fim">{{ __('Data Final') }}</label>
            <input type="text" name="data_fim" id="data_fim" class="form-control data-ptbr @error('data_fim') is-invalid @enderror" value="{{ $dataFim }}" placeholder="{{ __('dd/mm/aaaa') }}" autocomplete="off">
        </div>
    </div>
    <div class="col-md-3">
        <div class="form-group">
            <label for="hora_fim">{{ __('Hora Final') }}</label>
...
<div class="form-group">
    <label for="descricao">{{ __('Descrição / Agenda') }}</label>
    <textarea name="descricao" id="descricao" rows="4" class="form-control @error('descricao') is-invalid @enderror" placeholder="{{ __('Descreva a agenda, programação ou roteiro do evento') }}">{{ old('descricao', $evento->descricao) }}</textarea>
</div>
...
<div class="d-flex justify-content-between align-items-center mb-2">
    <h5 class="mb-0">{{ __('Equipe de Coordenação') }}</h5>
    <button type="button" class="btn btn-sm btn-outline-primary" id="add-equipe-row">{{ __('Adicionar Pessoa') }}</button>
</div>
<small class="text-muted d-block mb-3">{{ __('Marque apenas uma pessoa como líder. Se mais de uma for marcada, o sistema manterá a primeira.') }}</small>
...
<div id="equipe-container">
    @foreach ($equipeRows as $index => $membro)
        <div class="row equipe-row align-items-end mb-2">
            ...
            <div class="col-md-1">
                <button type="button" class="btn btn-sm btn-outline-danger remove-equipe-row">{{ __('Remover') }}</button>
            </div>
        </div>
    @endforeach
</div>
