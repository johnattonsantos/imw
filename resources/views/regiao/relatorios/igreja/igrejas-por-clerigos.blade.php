...
@section('breadcrumb')
    <x-breadcrumb :breadcrumbs="[
        ...
        ['text' => 'Igrejas por Clérigos', 'url' => '#', 'active' => true],
    ]"></x-breadcrumb>
@endsection
...
@section('content')
            ...
            <div class="widget-header">
                <div class="row">
                    <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                        <h4>{{ __('Relatório Igrejas por Clérigos') }}</h4>
                    </div>
                </div>
            </div>
            <div class="widget-content widget-content-area">
                <form class="form-vertical" id="filter_form" method="GET">
                    <div class="form-group row mb-4">
                        ...
                        <div class="col-lg-3">
                            <label class="control-label">{{ __('* Clérigo') }}</label>
                            <select class="form-control" id="clerigo" name="clerigo" required>
                                <option value="all">{{ __('Todos') }}</option>
                                @foreach($clerigos as $clerigo)
                                    <option value="{{ $clerigo->id }}" {{ request()->input('clerigo', 'all') == $clerigo->id ? 'selected' : '' }}>
                                        {{ $clerigo->nome }}
                                    </option>
                                @endforeach
                            </select>
    ...
    @if (request()->has('action'))
        <div class="col-lg-12 col-12 layout-spacing">
            <h6>IMW - RELATÓRIO IGREJAS POR CLÉRIGOS - REGIÃO {{ strtoupper($regiao->nome) }}</h6>
            <div class="statbox widget box box-shadow">
                <div class="widget-content widget-content-area">
                    <div class="table-responsive mt-0">
                        <table class="table table-bordered table-striped table-hover mb-4 display nowrap" id="igrejas-por-clerigos">
                            <thead>
                                <tr>
                                    <th>{{ __('DISTRITO') }}</th>
                                    <th>{{ __('IGREJA') }}</th>
...
@section('extras-scripts')
    ...
    <script>
        ...
        @if(request()->has('action'))
            new DataTable('#igrejas-por-clerigos', {
                scrollX: true,
                scrollY: 400,
                scrollCollapse: true,
                layout: {
                    topStart: {
                        buttons: [
                            'pageLength',
                            {
                                ...
                                title: 'IMW - RELATORIO IGREJAS POR CLERIGOS'
                            },
                            {
                                extend: 'pdf',
                                className: 'btn btn-primary btn-rounded',
