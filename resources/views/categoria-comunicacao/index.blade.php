...
<x-breadcrumb :breadcrumbs="[
    ...
    ['text' => 'Categoria Comunicação', 'url' => '/categoria-comunicacao', 'active' => true],
]"></x-breadcrumb>
@endsection
...
<div class="col-lg-12 col-12 layout-spacing">
    <div class="statbox widget box box-shadow">
        ...
        <div class="widget-content widget-content-area">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @forelse ($categorias as $categoria)
                            <tr id="row-categoria-{{ $categoria->id }}">
            ...
            {{ $categorias->links('vendor.pagination.index') }}
        </div>
    </div>
</div>

<div class="modal fade" id="modalCategoriaForm" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="formCategoriaAjax" novalidate>
...
<script>
    (function() {
        ...
        const storeUrl = "{{ route('categoria-comunicacao.store') }}";
        const showUrlTemplate = "{{ route('categoria-comunicacao.show', ['categoriaComunicacao' => '__ID__']) }}";
        const updateUrlTemplate = "{{ route('categoria-comunicacao.update', ['categoriaComunicacao' => '__ID__']) }}";
        const deleteUrlTemplate = "{{ route('categoria-comunicacao.destroy', ['categoriaComunicacao' => '__ID__']) }}";
        ...
        function loadCategoria(id, callback) {
            $.ajax({
        ...
        $('#btn-open-create').on('click', function() {
            currentMode = 'create';
            resetForm();
            $('#modalCategoriaFormTitle').text('Nova categoria');
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            loadCategoria(id, function(data) {
                $('#categoria-id').val(data.id);
                $('#categoria-nome').val(data.nome);
                $('#modalCategoriaFormTitle').text('Editar categoria');
                $('#modalCategoriaForm').modal('show');
            });
        });
        ...
        $(document).on('click', '.btn-delete', function() {
            ...
            swal({
                ...
                $.ajax({
                    ...
                    success: function(resp) {
                        $('#row-categoria-' + id).remove();
                        swal('Sucesso', resp.message || 'Categoria excluida com sucesso.', 'success');
                    },
                    error: function(xhr) {
        ...
        $('#formCategoriaAjax').on('submit', function(e) {
            ...
            if (!nome) {
                swal('Validação', 'O campo Nome é obrigatório.', 'error');
                return;
            }
            ...
            $.ajax({
                ...
                data: payload,
                success: function(resp) {
                    $('#modalCategoriaForm').modal('hide');
                    swal('Sucesso', resp.message || 'Operação realizada com sucesso.', 'success');
