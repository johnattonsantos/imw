...
<head>
    ...
    <script src="{{ asset('theme/assets/js/libs/jquery-3.1.1.js') }}"></script> <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery.mask/1.14.16/jquery.mask.min.js"></script>
    ...
    @php
        ...
        $traduzNomeInstituicaoPerfil = function ($nome) {
            $nome = trim((string) $nome);

            if (preg_match('/^(\d+)\s+REGI(?:Ã|A)O$/iu', $nome, $matches)) {
                return __(':numero REGIÃO', ['numero' => $matches[1]]);
            }
...
<body>
        ...
        <div class="col-xl-8 col-md-8 col-8">
            <div >
                <div class="card-content">
                    <img src="{{ asset('auth/images/login.png') }}" alt="{{ __('Logotipo') }}" class="logo mt-4 p-3">
                    <div class="card-body text-center">
                        <h5 class="mb-4"><b>{{ __('Olá :name, você precisa selecionar um perfil de acesso:', ['name' => trim($firstName . ' ' . $lastName)]) }}</b></h5>
                        @if (session('error'))
                        ...
                        @endif
                        @if ($perfils->isEmpty())
                            <p>{{ __('Nenhum perfil vinculado a este usuário.') }}</p>
                        @else
                                    ...
                                    <div class="col-lg-4 col-md-6 mb-4 ">
                                        <div class="card">
                                            <div class="card-header bg-primary">
                                                <h6 class="text-white text-bold-600"> {{ $traduzNomeInstituicaoPerfil($perfil->instituicao_nome) }}
                                                </h6>
                                            </div>
                                            <div class="card-body">
                                                <h6>{{ __($perfil->perfil_nome) }}</h6>
                                            </div>
    ...
    <script src="{{ asset('theme/bootstrap/js/popper.min.js') }}"></script> <script src="{{ asset('theme/bootstrap/js/bootstrap.min.js') }}"></script> <script src="https://cdnjs.cloudflare.com/ajax/libs/toastr.js/latest/toastr.min.js"></script>
</body>
