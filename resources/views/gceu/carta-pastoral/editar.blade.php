    ...
    <div class="statbox widget box box-shadow">
        <div class="widget-header">
            <div class="row">
                <div class="col-xl-12 col-md-12 col-sm-12 col-12">
                    <h4>{{ __('Editar carta pastoral') }}</h4>
                </div>
            </div>
        </div>
        <div class="widget-content widget-content-area">
            ...
            <form class="form-vertical" action="{{ route('gceu.carta-pastoral.update',$cartaPastoral->id) }}" id="form_create_gceu" method="post">
                ...
                <div class="row">
                    ...
                    <div class="form-group mb-4 col-12">
                        ...
                        <input type="text" name="titulo" class="form-control @error('titulo') is-invalid @enderror" id="titulo" required placeholder="{{ __('Título') }}" minlength="4" value="{{ $cartaPastoral->titulo }}" maxlength="150">
                        @error('titulo')
                            <div class="invalid-feedback">{{  $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group mb-4 col-12">
                        ...
                        <textarea name="conteudo" id="conteudo" class="form-control @error('conteudo') is-invalid @enderror" placeholder="{{ __('Introdução') }}">{{ $cartaPastoral->conteudo }}</textarea>
                        @error('conteudo')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="alert alert-warning" role="alert">
               {{ __('Carta pastoral não encontrada.') }}
            </div>
            @endif
        </div>
    </div>
