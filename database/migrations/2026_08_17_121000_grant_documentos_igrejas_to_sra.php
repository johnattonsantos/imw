...
return new class extends Migration
{
    ...
    {
        $now = now();
        $ruleId = DB::table('regras')->where('nome', 'documentos-igrejas-gerenciar')->value('id');

        if (!$ruleId) {
            $ruleId = DB::table('regras')->insertGetId([
                'nome' => 'documentos-igrejas-gerenciar',
                'created_at' => $now,
                'updated_at' => $now,
                'deleted_at' => null,
            ]);
        ...
        $profileIds = DB::table('perfils')
            ->whereIn('nome', ['Administrador Região', 'Administrador SRA'])
            ->pluck('id');
